...
# Iconos visuales
https_icon_label = tk.Label(frame_analizador, text="", font=("Consolas", 12), bg="#121212", fg="white")
https_icon_label.pack(pady=2)

sql_icon_label = tk.Label(frame_analizador, text="", font=("Consolas", 12), bg="#121212", fg="white")
sql_icon_label.pack(pady=2)
...
def ejecutar_analisis_mejorado(url):
    try:
        ...
        # Icono visual de HTTPS
        if "HTTPS habilitado" in https_check:
            https_icon_label.config(text="🔒 HTTPS Detectado", fg="green")
        else:
            https_icon_label.config(text="🔓 No tiene HTTPS", fg="red")

        # Icono visual de SQL Injection
        if vulnerabilidad_sql:
            sql_icon_label.config(text="⚠️ Vulnerable a SQL Injection", fg="orange")
        else:
            sql_icon_label.config(text="✅ No vulnerable a SQL Injection", fg="green")

        # Iconos visuales de encabezados de seguridad
        for encabezado in encabezados_criticos:
            if encabezado in r.headers:
                encabezado_visual_labels[encabezado].config(
                    text=f"✅ {encabezado}", fg="green"
                )
            else:
                encabezado_visual_labels[encabezado].config(
                    text=f"❌ {encabezado} no presente", fg="red"
                )

    except Exception as e:
        salida_analizador.insert(tk.END, f"Error: {e}\n")
        https_icon_label.config(text="❌ Error al analizar", fg="red")
        sql_icon_label.config(text="")
...
# Iconos visuales de encabezados de seguridad
encabezados_label = tk.Label(frame_analizador, text="Encabezados de Seguridad:", font=("Consolas", 12, "bold"), bg="#121212", fg="white")
encabezados_label.pack(pady=(10, 0))

encabezado_visual_labels = {}
encabezados_criticos = [
    "Strict-Transport-Security",
    "X-Content-Type-Options",
    "X-XSS-Protection",
...
for encabezado in encabezados_criticos:
    lbl = tk.Label(frame_analizador, text="", font=("Consolas", 11), bg="#121212", fg="white")
    lbl.pack()
    encabezado_visual_labels[encabezado] = lbl
...
def mostrar_sobre_mi():
    sobre_mi = """
------------------------
🚀 Acerca de NetPhantom 🚀
------------------------

Soy NetPhantom, un experto en ciberseguridad con más de 10 años de experiencia en el mundo digital. 
Mi identidad real es un misterio, ya que prefiero mantenerme en las sombras y enfocar mis esfuerzos 
en lo que realmente importa: fortalecer la seguridad en línea.

Desde mi base en España, he dedicado mi vida a comprender los complejos sistemas digitales, 
desarrollando estrategias avanzadas para protegerlos contra las amenazas cibernéticas. 
Con una trayectoria impecable, me he convertido en una figura influyente, pero siempre prefiero 
que mi trabajo hable por mí.

------------------------
"""
    return sobre_mi

# Mostrar "Sobre mí" en una ventana de texto
frame_sobre_mi = tk.Frame(notebook, bg="#121212")
notebook.add(frame_sobre_mi, text="🧑‍💻 Sobre mí")

salida_sobre_mi = tk.Text(frame_sobre_mi, height=15, width=80, bg="#222", fg="#00aa00", wrap=tk.WORD)
salida_sobre_mi.pack(pady=10)

# Insertar el texto "Sobre mí"
salida_sobre_mi.insert(tk.END, mostrar_sobre_mi())

# Configuración para que el texto sea solo de lectura
salida_sobre_mi.config(state=tk.DISABLED)
